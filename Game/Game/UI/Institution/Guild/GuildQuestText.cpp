	...
	namespace nsUI
	{
		...
		void CGuildQuestText::Select()
		{
			if (m_questProgress == EnQuestProgress::enNotReceive) //受注していない
			{
				...
				std::string logString = m_questName + "を受注しました。";
				logSystem->AddGameLog(logString, CVector4::Green());
			...
			else if (m_questProgress == EnQuestProgress::enReceive) //受注しているが達成していない
			{
				...
				std::string logString = m_questName + "は進行中です。";
				logSystem->AddGameLog(logString, CVector4::Green());
			}
			else //達成している
			{
				...
				std::string logString = m_questName + "をクリアしました。";
				logSystem->AddGameLog(logString, CVector4::Green());
